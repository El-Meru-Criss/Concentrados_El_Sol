...
CREATE TABLE IF NOT EXISTS `sol`.`vendedores` (
  ...
  `rol` BINARY(1) NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `sol`.`producto` (
  ...
  `peso` FLOAT NOT NULL,
  `Estado_Producto` TINYINT(1) NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `sol`.`inventario` (
  ...
  `stock_minimo` FLOAT NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `sol`.`inventario_has_ventas` (
  ...
  `cantidad_enviada` FLOAT NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `sol`.`proveedor_has_producto_has_pedidos_proveedor` (
  ...
  `recibidos` INT(11) NULL DEFAULT NULL,
...
-- -----------------------------------------------------
-- Table `sol`.`historial_abonos`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `sol`.`historial_abonos` (
  `idhistorial_abonos` INT NOT NULL,
  `cartera_idcartera` INT(11) NOT NULL,
  `cantidad_abonada` FLOAT NOT NULL,
  `fecha_abono` DATETIME NOT NULL,
  PRIMARY KEY (`idhistorial_abonos`),
  INDEX `fk_historial_abonos_cartera1_idx` (`cartera_idcartera` ASC),
  CONSTRAINT `fk_historial_abonos_cartera1`
    FOREIGN KEY (`cartera_idcartera`)
    REFERENCES `sol`.`cartera` (`idcartera`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
